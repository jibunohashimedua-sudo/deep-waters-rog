 ...
 * Depth — the profile.
 *
 * It already held progress, depths and history; what it never did was read
...
export default async function DepthPage() {
  ...
  // Share of each day's reading completed, 0 to 1. A day recorded as kept
  // is 1 whatever the ticks say — the completion is the record, the ticks
  // are only how most people get there. SoundingProfile rounds this into
  // its four steps; the arithmetic stays here where the data is.
  ...
  for (let d = 1; d <= 90; d++) {
    ...
    if (total > 0 && ticks > 0) shareByDay.set(d, Math.min(1, ticks / total));
  }
  ...
  const progress = (
    <>
      ...
      <section className="mt-10">
        ...
        <SoundingProfile days={shareByDay} today={day} />
        <p className="meta mt-3">{doneDays.size} kept</p>
      </section>

      {/* The four depths. Mono, no icon, no colour, no celebration — a
          depth you have reached is at full strength and one you have not
          is quiet, and nothing says how far away it is. */}
      <section className="mt-10">
        <h2 className="text-[15px] font-semibold tracking-[-0.01em] text-rog-ink">Depths</h2>
        <ul className="mt-4">
          {DEPTHS.map((d) => {
            const reached = doneDays.size >= d.at;
            return (
              <li key={d.at} className="depth-row">
                <span
                  className="meta"
                  style={{ color: reached ? "var(--ink-data)" : "var(--ink-quiet)" }}
                >
                  {d.name}
                </span>
                <span
